...
const { getLeads, getLeadById, insertLead, updateLead, getConversations, addConversation, getPackages, updatePackage, insertPackage, getSetting, setSetting, getOrAssignVariant, hasAnyLeadForChat, saveTelegramMessageId, getOldTelegramMessageIds, deleteTelegramMessageIdRecord } = require("./db");
...
/** Telegram'a mesaj gönderir, message_id'yi kaydeder; süre dolan mesajları sadece Telegram'dan siler (veritabanı kalır). */
 ...
 bot.on("callback_query", async (ctx) => {
  ...
  if (data.startsWith("arama_evet_")) {
   ...
   if (data && data.startsWith("cancel_and_menu_") && /^cancel_and_menu_[1-5]$/.test(data)) {
     const menuNum = data.replace("cancel_and_menu_", "");
     setChatState(chatId, null);
     if (menuNum === "1") {
